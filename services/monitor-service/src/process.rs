...
impl ProcStat {
    ...
    pub fn from_pid(pid: i32) -> Result<Self, failure::Error> {
        let file = File::open(Path::new(&format!("/proc/{}/stat", pid)))?;
    ...
    pub fn cmd(&self) -> Result<Vec<String>, failure::Error> {
        let file = File::open(Path::new(&format!("/proc/{}/cmdline", self.pid)))?;
...
pub fn running_pids() -> Vec<i32> {
    ...
    let entries = match fs::read_dir(Path::new("/proc")) {
        Ok(e) => e,
        Err(_) => return info,
    };
